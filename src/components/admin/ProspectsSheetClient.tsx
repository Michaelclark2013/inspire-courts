...
const STATUS_COLORS: Record<string, string> = {
  ...
  "no response": "bg-white text-text-secondary border-border",
  "no go": "bg-danger/10 text-danger border-danger/20",
  unknown: "bg-white text-text-secondary border-border",
};
            ...
            <thead>
              <tr className="border-b border-border bg-off-white">
                {([["team","Team"],["coach","Coach"],["division","Division"],["status","Status"],["date","Date"],["","Contact"],["notes","Notes"]] as const).map(([key, label]) => (
                  ...
                  >
                    {label}
                    {key && sortKey === key && <span className="ml-1 text-red">{sortDir === "asc" ? "\u25B2" : "\u25BC"}</span>}
