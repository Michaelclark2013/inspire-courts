...
export default function QuickContactBar({ subject, label = "Interested?" }: QuickContactBarProps) {
  return (
    <div className="fixed bottom-0 left-0 right-0 z-[90] bg-navy/95 backdrop-blur-sm border-t border-white/10 py-3 px-4 lg:hidden">
      <div className="flex items-center gap-3 max-w-lg mx-auto">
        ...
        <div className="flex gap-2 flex-1">
          <a
            href={`mailto:mikeyclark.240@gmail.com?subject=${encodeURIComponent(subject)}`}
          ...
          <Link
            href={`/contact?type=${encodeURIComponent(subject)}`}
