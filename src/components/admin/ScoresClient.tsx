    ...
    <>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        ...
        <div className="bg-bg-secondary border border-border rounded-sm p-4">
          ...
          <p className="text-2xl font-bold text-white mt-2">{events.length - 1}</p>
        </div>
        <div className="bg-bg-secondary border border-border rounded-sm p-4">
          <span className="text-text-secondary text-xs font-bold uppercase tracking-wider">Divisions</span>
          <p className="text-2xl font-bold text-white mt-2">{divisions.length - 1}</p>
        </div>
        <div className="bg-bg-secondary border border-border rounded-sm p-4">
          <span className="text-text-secondary text-xs font-bold uppercase tracking-wider">Courts Used</span>
          <p className="text-2xl font-bold text-white mt-2">{new Set(games.map((g) => g.court)).size}</p>
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-4">
        <div className="relative flex-1">
          ...
          <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search teams..." className="w-full bg-bg border border-border rounded-sm pl-10 pr-4 py-2.5 text-white text-sm focus:outline-none focus:border-accent transition-colors placeholder:text-text-secondary/50" />
        </div>
        <select value={eventFilter} onChange={(e) => setEventFilter(e.target.value)} className="bg-bg border border-border rounded-sm px-3 py-2.5 text-white text-sm focus:outline-none focus:border-accent">
          {events.map((e) => <option key={e} value={e}>{e === "All" ? "All Events" : e}</option>)}
        </select>
        <select value={divisionFilter} onChange={(e) => setDivisionFilter(e.target.value)} className="bg-bg border border-border rounded-sm px-3 py-2.5 text-white text-sm focus:outline-none focus:border-accent">
                ...
                <tbody>
                  {eventGames.map((g, i) => (
                    <tr key={i} className="border-b border-border/50 hover:bg-bg/50 transition-colors">
                      <td className={cn("px-4 py-3", g.winner === g.home ? "text-accent font-bold" : "text-white")}>{g.home}</td>
                      <td className={cn("px-4 py-3", g.winner === g.away ? "text-accent font-bold" : "text-white")}>{g.away}</td>
                      <td className="px-4 py-3 text-white font-medium font-mono">{g.score}</td>
                      <td className="px-4 py-3"><span className="bg-accent/10 text-accent text-xs px-2 py-0.5 rounded-sm">{g.division}</span></td>
                      <td className="px-4 py-3 text-white">{g.court}</td>
